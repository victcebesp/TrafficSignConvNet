...
images0, label0 = get_images_paths("data/test/0/*.ppm", 117), [0., 0., 1.]
images1, label1 = get_images_paths("data/test/1/*.ppm", 117), [0., 1., 0.]
images2, label2 = get_images_paths("data/test/2/*.ppm", 117), [1., 0., 0.]
...
with tf.variable_scope('ConvNet'):
    ...
    h = tf.layers.dense(inputs=tf.reshape(o4, [117, 18 * 18 * 64]), units=10, activation=tf.nn.relu)
...
with tf.Session() as sess:
    ...
    confusion_matrix0 = sess.run(tf.confusion_matrix(np.zeros(117), np.array(output0), 3))
    confusion_matrix1 = sess.run(tf.confusion_matrix(np.zeros(117) + 1, np.array(output1), 3))
    confusion_matrix2 = sess.run(tf.confusion_matrix(np.zeros(117) + 2, np.array(output2), 3))

    right_answers = np.diagonal(confusion_matrix0).sum() + \
                    np.diagonal(confusion_matrix1).sum() + \
                    np.diagonal(confusion_matrix2).sum()

    print "Acurracy: " + str(right_answers * 100 / (117 * 3)) + "%"